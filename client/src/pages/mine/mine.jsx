  ...
  _getDestineBooks() {
    ...
    return new Promise((resolve, reject) => {
      db.collection("booksInfo")
        ...
        .limit(LIMIT_COUNT)
        .skip(LIMIT_COUNT * currentDestineIndex)
        .orderBy("destineTime", "desc")
        .get()
        .then(res => {
          console.log(res.data);
      ...
      return (
        ...
        >
          <View className="info-des">
            ...
            <View className="des-right">
              <View
                onClick={this.contactOther.bind(
                  this,
                  bookItem.destineTel,
                  bookItem.bookStatus
                )}
    ...
    return (
      <View className="mine">
        ...
        <View className="mine-books">
          ...
          >
            <AtTabsPane current={this.state.currentPage} index={0}>
              <View>
                {BooksList}
                {!booksInfo.length && !this.state.isLoad && (
                  <View className="nothing-wrapper">您还没发布任何书籍</View>
                )}
            ...
            <AtTabsPane current={this.state.currentPage} index={1}>
              <View>
                {DestineBooksList}
                {!destineBooksInfo.length && !this.state.isLoad && (
                  <View className="nothing-wrapper">您还没预订任何书籍</View>
                )}
        ...
        <View className="model">
          <SeModal
            cancelIsOpen
            onCancelModalHandle={this.onCancelModalHandle}
            content={this.state.modalContent}
            isOpened={this.state.isModalOpened}
            onConfirmModalHandle={this.onConfirmModalHandle}
          ></SeModal>
        </View>
        <AtMessage />
      </View>
