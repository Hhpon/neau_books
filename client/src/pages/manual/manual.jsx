  ...
  putOutOne() {
    setTimeout(() => {
      ...
      if (
        ...
        this.state.isAuthorError ||
        this.state.isPriceError ||
        this.state.isNowPriceError ||
        !percentStatus ||
        !this.state.title ||
        !this.state.author ||
        !this.state.price ||
  ...
  render() {
    return (
      <View className="manual">
        <View>
          <SeInput
            onBlurInput={this.titleBlur}
            isError={this.state.isTitleError}
            value={this.state.title}
            title="书名"
            placeholder="请输入书名"
            onChangeInput={this.titleChange}
          ></SeInput>
          <SeInput
            onBlurInput={this.authorBlur}
            isError={this.state.isAuthorError}
            value={this.state.author}
            title="主编"
            placeholder="请输入主编"
            onChangeInput={this.authorChange}
          ></SeInput>
          <SeInput
            onBlurInput={this.priceBlur}
            isError={this.state.isPriceError}
            value={this.state.price}
            type="number"
            title="原价"
            placeholder="请输入原价"
            onChangeInput={this.priceChange}
          ></SeInput>
          <SeInput
            onBlurInput={this.nowPriceBlur}
            isError={this.state.isNowPriceError}
            value={this.state.nowPrice}
            title="现价"
            type="number"
            placeholder="请输入现价"
            onChangeInput={this.nowPriceChange}
          ></SeInput>
          <SeInput
            isClick
            onBlurInput={this.percentStatusBlur}
            isError={this.state.isPercentStatusError}
            value={this.state.percentStatus}
            disabled
            title="品相"
            placeholder="请输入品相"
            onClickInput={this.percentClick}
          ></SeInput>
          <View style="margin: 20px 10px;">
            <AtButton type="primary" onClick={this.putOutOne}>
              确认发布
            </AtButton>
          </View>
          ...
          <View className="modal">
            <SeModal
              onCancelModalHandle={this.putOutCancelModalHandle}
              cancelIsOpen={this.state.cancelIsOpen}
              content={this.state.putOutModalContent}
              isOpened={this.state.isPutOutModalOpened}
              onConfirmModalHandle={this.putOutConfirmModalHandle}
            ></SeModal>
          </View>
          <AtMessage />
        </View>
