...
export default class Attest extends Component {
  ...
  attestInfo() {
    setTimeout(() => {
      if (
        this.state.isStudentIDError ||
        this.state.isTelError ||
        this.state.isCharCodeError ||
  ...
  render() {
    return (
      <View className="attest">
        ...
        <View className="attest-remark">
          注：学生认证为了保证买卖书籍同学均为东农学子，所以学号密码为教务系统的学号密码！
        </View>
        <SeInput
          isError={this.state.isStudentIDError}
          onBlurInput={this.studentIDBlur}
          title="学号"
          value={this.state.studentID}
          placeholder="请输入学号"
          onChangeInput={this.studentIDChange}
          maxlength={9}
        ></SeInput>
        <SeInput
          isError={this.state.isStudentPassWordError}
          title="密码"
          value={this.state.studentPassWord}
          placeholder="请输入密码"
          onChangeInput={this.studentPassWord}
        ></SeInput>
        <SeInput
          isError={this.state.isTelError}
          onBlurInput={this.telBlur}
          type="number"
          title="联系电话"
          value={this.state.tel}
          placeholder="请输入联系电话"
          onChangeInput={this.telChange}
          maxlength={11}
        ></SeInput>
        <View className="charcode-container">
          <SeInput
            isError={this.state.isCharCodeError}
            title="验证码"
            value={this.state.charCode}
            onBlurInput={this.charCodeBlur}
            onChangeInput={this.charCodeChange}
            placeholder="请输入验证码"
            maxlength={4}
          ></SeInput>
        ...
        <AtButton onClick={this.attestInfo} type="primary">
          提交
        </AtButton>
        <View className="modal">
          <SeModal
            cancelIsOpen
            content={this.state.attestModalContent}
            isOpened={this.state.isAttestModalOpened}
